...
/**
 * 启用@ConfigurationProperties注解的ImportSelector
 */
class EnableConfigurationPropertiesImportSelector implements ImportSelector {

	private static final String[] IMPORTS = {
			//这个主要是向spring中注册@EnableConfigurationProperties
			//所以@EnableConfigurationProperties会向spring中注入就是在这个类完成的
			ConfigurationPropertiesBeanRegistrar.class.getName(),
			//这个主要是解析@ConfigurationProperties注解，向其中填充值
	 ...
	 * @ConfigurationProperties 注解bean注册 这个import实现了ImportBeanDefinitionRegistrar接口
	 * 所以会调用registerBeanDefinitions方法
	 */
	public static class ConfigurationPropertiesBeanRegistrar
		...
		public void registerBeanDefinitions(AnnotationMetadata metadata,
			...
			//遍历@EnableConfigurationProperties注解中的类，并向spring注册
			getTypes(metadata).forEach((type) -> register(registry,
					(ConfigurableListableBeanFactory) registry, type));
		}

		/**
		 * 获取@EnableConfigurationProperties注解对应的类
		 * @param metadata
		 * @return
		 */
		...
		private void register(BeanDefinitionRegistry registry,
			...
			if (!containsBeanDefinition(beanFactory, name)) {
				//向BeanDefinition中注册ConfigurationProperties的bean
		...
		private String getName(Class<?> type) {
			...
			//获取ConfigurationProperties前缀+"-"+EnableConfigurationProperties注解中的类名
